...
import simulators.FHashFlow as FHashFlow
...
def func(hf, flows):
	...
	res["are"] = dict()
	res["f1score"] = dict()
	print("n_promotions:%d" % len(hf.records))
...
def calc(n_pkts):
	...
	fhf = FHashFlow.FHashFlow(src, TYPE_JSON, n_pkts)
#	for 流标识符, 计数器 in fhf.M:
#		print(流标识符, 计数器)
	print("HashFlow:")
	...
	with open(filename, "w") as f:
		...
		json.dump(res, f)
	print("FHashFlow:")
	filename = ".".join(["fhf", "npkts", str(n_pkts), "caida.130000.json"])
	with open(filename, "w") as f:
		res = func(fhf, cls.flows)
		json.dump(res, f)

if "__main__" == __name__:
#	for n_pkts in range(1, 11):
#		n_pkts = n_pkts * 500000
#		calc(n_pkts)
	calc(n_pkts)
