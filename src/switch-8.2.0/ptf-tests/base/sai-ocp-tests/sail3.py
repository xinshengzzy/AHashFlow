    ...
    def runTest(self):
        ...
        try:
            for i in range(128):
                ...
                mask=""
                send_packet(self, 2, str(pkt))
                verify_no_packet(self, exp_pkt, 1, timeout=2)
        finally:
            if mask!="":
                sai_thrift_remove_route(self.client, vr_id, addr_family, net, mask, None)
    ...
    def runTest(self):
        ...
        try:
            ...
            exp_pkt2 = simple_tcp_packet(
                                ...
                                ip_ttl=63)
            send_packet(self, 2, str(pkt))
            verify_any_packet_any_port(self, [exp_pkt1, exp_pkt2], [0, 1], timeout=2)
...
class L3IPv4EcmpLpmTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        print
        print "Sending packet port 3 -> port [0,1,2] (192.168.0.1 -> 172.17.10.1 [id = 101])"
        switch_init(self.client)
        port1 = port_list[0]
        port2 = port_list[1]
        port3 = port_list[2]
...
class L3IPv4LagTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        sai_thrift_remove_default_bridge_ports(self.client, [port3])
        ...
        try:
            ...
            sai_thrift_create_default_bridge_ports(self.client, [port3])
...
class L3EcmpLagTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        sai_thrift_vlan_remove_ports(self.client, switch.default_vlan.oid, [port1, port2, port3, port4, port5])
        lag_member_id1 = sai_thrift_create_lag_member(self.client, lag_id1, port1)
        lag_member_id2 = sai_thrift_create_lag_member(self.client, lag_id1, port2)
        ...
        sai_thrift_remove_default_bridge_ports(self.client, [port6, port7])
        ...
        try:
            count = [0, 0, 0, 0, 0, 0]
            dst_ip = int(socket.inet_aton('10.20.10.1').encode('hex'), 16)
            ...
            for i in range(3, 5):
                self.assertTrue((count[i] >= ((max_itrs / 6) * 0.72)),
            ...
            sai_thrift_create_default_bridge_ports(self.client, [port6, port7])
...
class L3EcmpLagTestMini(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        sai_thrift_remove_default_bridge_ports(self.client, [port3, port4])
        ...
        try:
            ...
            sai_thrift_create_default_bridge_ports(self.client, [port3, port4])
...
class L3VIIPv4HostTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            bport_oid = sai_thrift_create_bridge_port(self.client, SAI_BRIDGE_PORT_TYPE_PORT, port2)
            vlan_member1 = sai_thrift_create_vlan_member(self.client, switch.default_vlan.oid, port2, SAI_VLAN_TAGGING_MODE_UNTAGGED)
...
class L3MultipleLagTest(sai_base_test.ThriftInterfaceDataPlane):
    ...
    def setup_lags(self, num_of_lags, port_list):
        ...
        for i in xrange(num_of_lags):
            sai_thrift_create_neighbor(self.client, self.addr_family, self.lags_rifs[i], "10.91.%s.1" % str(i+1), self.mac_pool[i])
            sai_thrift_create_route(self.client, self.vr_id, self.addr_family, "10.91.%s.0" % str(i+1), self.ip_mask, self.lags_rifs[i])

    def teardown_lags(self, num_of_lags, port_list):
        if (num_of_lags == 0 ): return
        for i in xrange(num_of_lags):
            sai_thrift_remove_neighbor(self.client, self.addr_family, self.lags_rifs[i], "10.91.%s.1" % str(i+1), self.mac_pool[i])
            sai_thrift_remove_route(self.client, self.vr_id, self.addr_family, "10.91.%s.0" % str(i+1), self.ip_mask, self.lags_rifs[i])
    ...
    def send_and_verify_packets(self, num_of_lags, port_list):
        ...
        for i in xrange(NUM_OF_PKTS_TO_SEND):
                ip_src = '10.80.' + str(i % 255) + '.' + str(i % 255)
                ip_dst = '10.91.' + str((i % num_of_lags) + 1) + '.1'
                src_mac = src_mac_start.format(str(i).zfill(4)[:2], str(i).zfill(4)[2:])
                pkt = simple_tcp_packet(eth_dst=router_mac,
                                        eth_src=src_mac,
                                        ip_src=ip_src,
                ...
                (match_index,rcv_pkt) = verify_packet_any_port(self,exp_pkt,destanation_ports, timeout=2)
...
class L3MultipleEcmpLagTest(sai_base_test.ThriftInterfaceDataPlane):
    ...
    def setup_ecmp_lag_group(self, first_rif_port):
        ...
        #adding lag members
        sai_thrift_vlan_remove_ports(self.client, switch.default_vlan.oid, [port_list[1]])
        ...
        for i in range(self.first_changing_port,first_rif_port):
            sai_thrift_vlan_remove_ports(self.client, switch.default_vlan.oid, [port_list[i]])
        ...
        sai_thrift_create_neighbor(self.client, self.addr_family, self.lag_rif, "10.91.0.1", self.mac_pool[self.total_changing_ports])
        sai_thrift_create_route(self.client, self.vr_id,self.addr_family, "10.91.0.1", '255.255.255.0', self.lag_rif)
        self.nhops.append(sai_thrift_create_nhop(self.client, self.addr_family, "10.91.0.1" , self.lag_rif))
        for i in range(first_rif_port,self.total_changing_ports):
            sai_thrift_remove_default_bridge_ports(self.client, [port_list[i]])
        ...
        for i in range(len(self.port_rifs)):
            sai_thrift_create_neighbor(self.client, self.addr_family, self.port_rifs[i], "10.91.%s.1" % str(i+1), self.mac_pool[i])
            self.nhops.append(sai_thrift_create_nhop(self.client, self.addr_family, "10.91.%s.1" % str(i+1), self.port_rifs[i]))
            sai_thrift_create_route(self.client, self.vr_id, self.addr_family, "10.91.%s.1" % str(i+1), '255.255.255.0', self.port_rifs[i])
        ...
        sai_thrift_create_route(self.client, self.vr_id, self.addr_family, "10.81.0.0", self.ip_mask, self.nhop_group)

    def teardown_ecmp_lag_group(self, first_rif_port):
        sai_thrift_remove_route(self.client, self.vr_id, self.addr_family, "10.81.0.0", self.ip_mask, self.nhop_group)
        sai_thrift_remove_route(self.client, self.vr_id, self.addr_family, "10.91.0.1", '255.255.255.0', self.lag_rif)
        sai_thrift_remove_neighbor(self.client, self.addr_family, self.lag_rif, "10.91.0.1", self.mac_pool[self.total_changing_ports])
        for i in range(self.total_changing_ports - first_rif_port):
            sai_thrift_remove_route(self.client, self.vr_id, self.addr_family, "10.91.%s.1" % str(i+1), '255.255.255.0', self.port_rifs[i])
        for nhop_gmember in self.nhop_gmembers:
            self.client.sai_thrift_remove_next_hop_group_member(nhop_gmember)
        for i in range(self.total_changing_ports - first_rif_port):
            sai_thrift_remove_neighbor(self.client, self.addr_family, self.port_rifs[i], "10.91.%s.1" % str(i+1), self.mac_pool[i])
        for nhop in self.nhops:
        ...
        for rif in self.port_rifs:
            self.client.sai_thrift_remove_router_interface(rif)
        for i in range(first_rif_port,self.total_changing_ports):
            sai_thrift_create_default_bridge_ports(self.client, [port_list[i]])
        del self.port_rifs[:]
        for lag_member in self.lag_members:
    ...
    def polarizationCheck(self,packets,avg):
        self.assertTrue((packets >= (avg * 0.2)),"Not all paths are equally balanced, %s" % packets)
        self.assertTrue((packets <= (avg * 4.00)),"Not all paths are equally balanced, %s" % packets)
    ...
    def send_and_verify_packets(self, first_rif_port):
        ...
        for i in xrange(IP_LAST_WORD_RANGE):
                for j in xrange(IP_2ND_LAST_WORD_RANGE):
                    ...
                    send_packet(self, 0, str(pkt))
                    (match_index,rcv_pkt) = verify_packet_any_port(self,masked_exp_pkt,destanation_ports, timeout=2)
                    logging.debug("Found expected packet from port %d" % destanation_ports[match_index])
                    pkt_counter[match_index] += 1
                    #sport = random.randint(0,0xffff)
                    #dport = random.randint(0,0xffff)
        ...
        lag_average = lag_packets/(len(self.lag_members))
        ...
        rifs_average = sum(pkt_counter)/(len(self.port_rifs))
    ...
    def runTest(self):
        ...
        self.vr_id = sai_thrift_create_virtual_router(self.client, self.v4_enabled, self.v6_enabled)
        sai_thrift_remove_default_bridge_ports(self.client, [self.src_port])
        rif_port_id = sai_thrift_create_router_interface(self.client, self.vr_id, 1, self.src_port, 0, self.v4_enabled, self.v6_enabled, '')
        ...
        try:
            ...
            self.client.sai_thrift_remove_router_interface(rif_port_id)
            sai_thrift_create_default_bridge_ports(self.client, [self.src_port])
            self.client.sai_thrift_remove_virtual_router(self.vr_id)
            print "END OF TEST"
