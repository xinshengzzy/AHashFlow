...
switch_status_t sai_port_counter_to_switch_port_counter(
    sai_port_stat_t port_stat_id, switch_port_counter_id_t *counter_id) {
...
sai_status_t sai_set_port_attribute(_In_ sai_object_id_t port_id,
  ...
  switch (attr->id) {
    ...
    case SAI_PORT_ATTR_GLOBAL_FLOW_CONTROL_MODE:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set link pause for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_PRIORITY_FLOW_CONTROL:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set priority pause for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
      ...
      if (pfc_set) {
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to set pfc cos mapping for port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to enable pfc on port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
    ...
    case SAI_PORT_ATTR_SPEED:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set speed for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_MTU:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set MTU for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_QOS_DOT1P_TO_TC_MAP:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set pcp trust for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_EGRESS_ACL: {
      ...
      if (acl_table_handle == SAI_NULL_OBJECT_ID) {
        if (attr->id == SAI_PORT_ATTR_INGRESS_ACL) {
          ...
          if (status != SAI_STATUS_SUCCESS) {
            SAI_LOG_ERROR("failed to get ingress acl handle for port %d: %s",
                          (port_handle & 0xFFFF),
                          sai_status_to_string(status));
            return status;
          }
          ...
          switch_status = switch_api_port_egress_acl_group_get(
              device, port_handle, &acl_table_handle);
          if (status != SAI_STATUS_SUCCESS) {
            SAI_LOG_ERROR("failed to get egress acl handle for port %d: %s",
                          (port_handle & 0xFFFF),
                          sai_status_to_string(status));
            return status;
          }
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to bind port to acl for port %d: %s",
                      (port_handle & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_ADMIN_STATE:
      switch_status =
          switch_api_port_admin_state_set(device, port_id, attr->value.oid);
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set admin state for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_BROADCAST_STORM_CONTROL_POLICER_ID:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set storm control policer for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_INGRESS_SAMPLEPACKET_ENABLE:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set sflow ingress for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_EGRESS_SAMPLEPACKET_ENABLE:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set sflow egress for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_INTERNAL_LOOPBACK_MODE:
      switch_status = switch_api_port_loopback_mode_set(
          device, port_id, sai_loopback_mode_to_switch_api(attr->value.oid));
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set sflow ingress for port %d: %s",
                      (port_id & 0xFFFF),
    ...
    case SAI_PORT_ATTR_AUTO_NEG_MODE:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set autoneg mode for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_FEC_MODE:
      fec_mode = sai_fec_mode_to_switch_fec(attr->value.u32);
      switch_status = switch_api_port_fec_mode_set(device, port_id, fec_mode);
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("Failed to set port fec mode for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_INGRESS_MIRROR_SESSION:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("Failed to set ingress port mirror for prot %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_EGRESS_MIRROR_SESSION:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("Failed to set egress port mirror for prot %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_QOS_SCHEDULER_PROFILE_ID:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("Failed to set port scheduler for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_DROP_UNTAGGED:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set drop_untagged attribute for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
    ...
    case SAI_PORT_ATTR_DROP_TAGGED:
      ...
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set drop_tagged attribute for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
      break;
    default:
      SAI_LOG_ERROR("Port Attr Set Not supported %d %s\n",
                    attr->id,
    ...
    switch (attr->id) {
      ...
      case SAI_PORT_ATTR_HW_LANE_LIST:
        ...
        if (status != SWITCH_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get port %d lane list: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_ADMIN_STATE:
        ...
        if ((status = sai_switch_status_to_sai_status(switch_status)) !=
            SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get port %d admin state: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_QOS_EGRESS_BUFFER_PROFILE_LIST:
        ...
        if ((status = sai_switch_status_to_sai_status(switch_status)) !=
            SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get max queues for port %d admin state: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
        ...
        if ((status = switch_api_queues_get(device,
                                            ...
                                            queue_handles)) !=
            SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to retrieve queue handle list for port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
      ...
      case SAI_PORT_ATTR_BROADCAST_STORM_CONTROL_POLICER_ID:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get storm control policer for port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_INTERNAL_LOOPBACK_MODE:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get loopback mode for port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_PRIORITY_FLOW_CONTROL:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get pfc for port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_AUTO_NEG_MODE:
        ...
        attr->value.booldata =
            (auto_neg_mode == SWITCH_PORT_AUTO_NEG_MODE_ENABLE) ? true : false;
        break;
      ...
      case SAI_PORT_ATTR_FEC_MODE:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to get FEC mode for port %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_QOS_TC_TO_PRIORITY_GROUP_MAP:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("failed to get port qosmap for port %lx: %s",
                        port_id,
                        sai_status_to_string(status));
          return status;
        }
        ...
        if (qos_handle) {
          ...
          status = sai_switch_status_to_sai_status(switch_status);
          if (status != SAI_STATUS_SUCCESS) {
            SAI_LOG_ERROR("failed to get qosmap type for port %lx: %s",
                          port_id,
                          sai_status_to_string(status));
            return status;
          }
        ...
        if (attr->id == SAI_PORT_ATTR_QOS_INGRESS_BUFFER_PROFILE_LIST) {
          for (i = 0; i < num_ppgs; i++) {
            ...
            if (ppg_profile_handle == SWITCH_API_INVALID_HANDLE) {
              SAI_LOG_ERROR("PPG profile handle is null for port 0x%lx",
                            port_id);
            }
      ...
      case SAI_PORT_ATTR_INGRESS_MIRROR_SESSION:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to get ingress port mirror for prot %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_EGRESS_MIRROR_SESSION:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to get egress port mirror for prot %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_INGRESS_SAMPLEPACKET_ENABLE:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to get ingress sflow handle for prot %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_EGRESS_SAMPLEPACKET_ENABLE:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to get egress sflow handle for prot %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      case SAI_PORT_ATTR_QOS_SCHEDULER_PROFILE_ID:
        ...
        status = sai_switch_status_to_sai_status(switch_status);
        if (status != SAI_STATUS_SUCCESS) {
          SAI_LOG_ERROR("Failed to get scheduler handle for prot %d: %s",
                        (port_id & 0xFFFF),
                        sai_status_to_string(status));
          return status;
        }
      ...
      default:
        SAI_LOG_ERROR("Port Attr Get Not supported %d %s\n",
                      attr->id,
                      port_attr_map[attr->id]);
        status = SAI_STATUS_NOT_SUPPORTED;
        break;
...
sai_status_t sai_create_port(_Out_ sai_object_id_t *port_id,
  ...
  *port_id = SAI_NULL_OBJECT_ID;
  ...
  status = sai_switch_status_to_sai_status(switch_status);
  if (status != SAI_STATUS_SUCCESS) {
    SAI_LOG_ERROR("create port failed: %s", sai_status_to_string(status));
    return status;
  }
  if (auto_neg_mode != SWITCH_PORT_AUTO_NEG_MODE_DEFAULT) {
    ...
    status = sai_switch_status_to_sai_status(switch_status);
    if (status != SAI_STATUS_SUCCESS) {
      SAI_LOG_ERROR("Failed to set autoneg mode for port 0x%lx: %s",
                    port_handle,
                    sai_status_to_string(status));
      return status;
    }
...
sai_status_t sai_remove_port(_In_ sai_object_id_t port_id) {
  ...
  status = sai_switch_status_to_sai_status(switch_status);
  if (status != SAI_STATUS_SUCCESS) {
    SAI_LOG_ERROR("remove port failed: %s", sai_status_to_string(status));
    return status;
  }
  return status;
}
...
sai_status_t sai_get_port_stats(_In_ sai_object_id_t port_id,
  ...
  for (index = 0; index < number_of_counters; index++) {
    status = sai_port_counter_to_switch_port_counter(counter_ids[index],
                                                     &switch_counter_id);
    if (status != SAI_STATUS_SUCCESS) {
      //      SAI_LOG_ERROR("sai get port stats for prt 0x%x: %s", port_id,
      //      sai_status_to_string(status));
...
sai_status_t sai_clear_port_stats(_In_ sai_object_id_t port_id,
  ...
  for (index = 0; index < number_of_counters; index++) {
    status = sai_port_counter_to_switch_port_counter(counter_ids[index],
                                                     &switch_counter_id);
    if (status != SAI_STATUS_SUCCESS) {
      SAI_LOG_ERROR("sai get port stats: %s", sai_status_to_string(status));
      SAI_FREE(switch_counter_ids);
      return status;
    }
    switch_counter_ids[index] = switch_counter_id;
...
sai_status_t sai_clear_port_all_stats(_In_ sai_object_id_t port_id) {
  ...
  status = sai_switch_status_to_sai_status(switch_status);
  if (status != SAI_STATUS_SUCCESS) {
    SAI_LOG_ERROR("sai clear all port stats failed 0x%lx: %s",
                  port_id,
                  sai_status_to_string(status));
    return status;
  }
