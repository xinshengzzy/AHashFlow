...
/******************************************************************************
 *
 * Test multiple instructions but no index
 *
 *****************************************************************************/
table two_instr_no_idx {
    reads { ig_intr_md.ingress_port : exact; }
    actions { add_9; rsub_17; }
    size : 17;
}
action add_9() {
    two_instr_no_idx_alu_1.execute_stateful_alu(ig_intr_md.ingress_port);
    modify_field(ig_intr_md_for_tm.drop_ctl, 0, 1);
    modify_field(ig_intr_md_for_tm.ucast_egress_port, ig_intr_md.ingress_port);
}
action rsub_17() {
    two_instr_no_idx_alu_2.execute_stateful_alu(ig_intr_md.ingress_port);
    modify_field(ig_intr_md_for_tm.drop_ctl, 0, 1);
    modify_field(ig_intr_md_for_tm.ucast_egress_port, ig_intr_md.ingress_port);
}
blackbox stateful_alu two_instr_no_idx_alu_1  {
    reg: two_instr_no_idx_reg;
    update_lo_1_value: register_lo + 9;
}
blackbox stateful_alu two_instr_no_idx_alu_2  {
    reg: two_instr_no_idx_reg;
    update_lo_1_value: 17 - register_lo;
}
register two_instr_no_idx_reg {
    width : 8;
    instance_count: 512;
}
