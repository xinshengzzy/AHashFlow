...
class L3IPv4QosDscpRewriteTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        rif_id1 = sai_thrift_create_router_interface(
            self.client, SAI_ROUTER_INTERFACE_TYPE_PORT,
            vr_id, port1, 0, v4_enabled, v6_enabled, mac)
        rif_id2 = sai_thrift_create_router_interface(
            self.client, SAI_ROUTER_INTERFACE_TYPE_PORT,
            vr_id, port2, 0, v4_enabled, v6_enabled, mac)
        ...
        nhop1 = sai_thrift_create_nhop(self.client, addr_family, ip_addr1,
                                       rif_id1)
        sai_thrift_create_route(self.client, vr_id, addr_family, ip_addr1,
                                ip_mask1, nhop1)
        ...
        tc_qos_map_id = sai_thrift_create_qos_map(
            self.client, SAI_QOS_MAP_TYPE_TC_TO_QUEUE, ingress_tc_list,
            ingress_queue_list)

        egress_tc_and_color_list = [[11, 0], [12, 0], [
            13,
            0,
        ], [14, 0]]
        egress_dscp_list = [10, 20, 30, 40]
        egress_qos_map_id = sai_thrift_create_qos_map(
            self.client, SAI_QOS_MAP_TYPE_TC_AND_COLOR_TO_DSCP,
            egress_tc_and_color_list, egress_dscp_list)
        ...
        sai_thrift_set_port_attribute(
            self.client, port1, SAI_PORT_ATTR_QOS_TC_AND_COLOR_TO_DSCP_MAP,
            egress_qos_map_id)

        sai_thrift_set_port_attribute(self.client, port2,
                                      SAI_PORT_ATTR_QOS_DSCP_TO_TC_MAP,
                                      ingress_qos_map_id)
        sai_thrift_set_port_attribute(self.client, port2,
                                      SAI_PORT_ATTR_QOS_TC_TO_QUEUE_MAP,
                                      tc_qos_map_id)
        sai_thrift_set_port_attribute(
            self.client, port2, SAI_PORT_ATTR_QOS_TC_AND_COLOR_TO_DSCP_MAP,
            egress_qos_map_id)
        ...
        try:
            ...
            sai_thrift_remove_neighbor(self.client, addr_family, rif_id1,
                                       ip_addr1, dmac1)
            sai_thrift_remove_route(self.client, vr_id, addr_family, ip_addr1,
                                    ip_mask1, nhop1)
