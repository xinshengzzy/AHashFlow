...
#else
    /* store change in len, not actual len */
...
action update_udp_checksum() {
    modify_field(nat_metadata.update_udp_checksum, TRUE);
}

action update_tcp_checksum() {
    modify_field(nat_metadata.update_tcp_checksum, TRUE);
}

action update_inner_udp_checksum() {
    modify_field(nat_metadata.update_inner_udp_checksum, TRUE);
}

action update_inner_tcp_checksum() {
    modify_field(nat_metadata.update_inner_tcp_checksum, TRUE);
}
...
table update_l4_checksum {
    ...
    size : UPDATE_L4_CHECKSUM_TABLE_SIZE;
}
#endif /* NAT_DISABLE */
