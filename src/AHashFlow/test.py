...
with open("./a_table.txt", "w") as f:
    for idx in range(a_table_size):
        value = client.register_read_a_table(sess_hdl, dev_tgt, idx, flag)
        f.write(str(value) + "\n")

with open("./b_table.txt", "w") as f:
    for idx in range(b_table_size):
        value = client.register_read_b_table(sess_hdl, dev_tgt, idx, flag)
        f.write(str(value) + "\n")
exit()
n = 0
for item in records:
